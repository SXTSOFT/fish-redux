...
Effect<T> mergeEffects<T extends K, K>(Effect<K> sup, [Effect<T> sub]) {
  return (FAction action, Context<T> ctx) {
...
typedef SubEffect<T> = FutureOr<void> Function(FAction action, Context<T> ctx);
...
HigherEffect<T> asHigherEffect<T>(Effect<T> effect) => effect != null
    ? (Context<T> ctx) => (FAction action) => effect(action, ctx)
    : null;
